...
if (seedDemoData && ownerEmail && ownerAccount && db.state.stores[0] && sellerAccount) {
...
function registeredRestaurant(store) {
  ...
  return {
    ...
    benefits: [],
    menu: grouped.size ? [...grouped].map(([name, items]) => ({ name, items })) : [{ name: 'Cardápio', items: [] }],
  }
}
...
function marketplaceOffers() {
  return db.state.promotions
    .filter((item) => item.active && (!item.endsAt || Date.parse(item.endsAt) >= Date.now()))
    .map((item) => {
  ...
  'POST /api/auth/forgot-password': async (params, query, body, ctx) => {
    ...
    if (!mailer.isConfigured()) {
      return {
        status: 503,
        body: {
          error: 'A recuperação por e-mail está temporariamente indisponível. Fale com o suporte.',
        },
      }
    }
  ...
  'POST /api/auth/reset-password': (params, query, body, _ctx) => {
    ...
    if (!rec) {
      return {
        ...
        body: {
          code: 'INVALID_TOKEN',
          error: 'Este link de redefinição é inválido ou já expirou. Solicite um novo.',
        },
      }
    }
...
const api = {
    ...
    return {
      ...
      sections: [
        ...
        },
        {
          ...
          restaurants: all.filter((r) => r.deliveryFee === 0 || r.freeShippingMin > 0).map(restaurantCard),
        },
        {
          id: 'offers',
        ...
        },
        {
          ...
          restaurants: [...all].sort((a, b) => b.rating - a.rating).map(restaurantCard),
        },
        {
          id: 'near',
          title: 'Perto de você',
        ...
        },
        {
          ...
          restaurants: all.filter((r) => ['taco-loco', 'market-express'].includes(r.id)).map(restaurantCard),
        },
      ],
    }
  },
  ...
  'GET /api/search': (params, query) => {
    ...
    const nq = normalize(q)
    const all = marketplaceRestaurants()
    const restaurants = all.filter((r) => !nq || normalize(r.name + ' ' + r.category + ' ' + r.tags.join(' ')).includes(nq)).map(restaurantCard)
    const products = q ? searchProducts(q, all) : []
    const suggestions = data.categories.filter((c) => !nq || normalize(c.name).includes(nq))
...
function createSimplePdf(title, subtitle, sections) {
  ...
  const newPage = () => {
    ...
    text(`RELATORIO  ${String(pageNumber).padStart(2, '0')}`, 480, 799, 8, true, '1 1 1')
  ...
  const tableRows = (rows) => {
    ...
    for (let index = 0; index < rows.length; index++) {
      ...
      if (empty) {
        ...
        commands.push(`0.72 0.79 0.74 RG [4 3] 0 d 34 ${y - 80} 527 86 re S [] 0 d`)
        text('SEM DADOS NESTE PERIODO', 205, y - 36, 10, true, '0.40 0.47 0.42')
      ...
      const cells = row.split('|').map((cell) => cell.trim()),
        height = 34
      rect(34, y - height + 4, 527, height, header ? '0.89 0.96 0.91' : index % 2 ? '0.97 0.98 0.97' : '1 1 1')
      commands.push(`0.83 0.89 0.85 RG 34 ${y - height + 4} 527 ${height} re S`)
      const usable = 505,
        width = usable / Math.max(1, cells.length)
      cells.forEach((cell, column) => text(cell, 45 + column * width, y - 18, 7.5, header || column === 0, header ? '0.05 0.42 0.21' : '0.18 0.23 0.20'))
      y -= height
    }
  }
  newPage()
  ...
  for (let index = 0; index < pages.length; index++) {
    const footer = ['0.91 0.94 0.92 rg', '34 43 527 1 re f', `0.40 0.46 0.42 rg BT /F1 7 Tf 34 27 Td (Documento gerado em ${pdfText(generated)} | Dados exclusivos do estabelecimento) Tj ET`, `0.05 0.42 0.21 rg BT /F2 8 Tf 522 27 Td (${index + 1} / ${pages.length}) Tj ET`].join('\n')
    pages[index] += '\n' + footer
  }
  const objects = [null, '<< /Type /Catalog /Pages 2 0 R >>', null, '<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>', '<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold >>']
  const pageIds = [],
    contentIds = []
...
function partnerReport(store, type) {
  const orders = db.state.platformOrders.filter((order) => order.storeId === store.id),
    promotions = db.state.promotions.filter((item) => item.storeId === store.id),
    finance = platform.finance(store.id),
    date = new Date().toLocaleDateString('pt-BR')
  if (type === 'financeiro')
    return createSimplePdf('Relatorio financeiro', `${store.name} | Posicao em ${date}`, [
      {
        title: 'Resumo',
        rows: [`Vendas brutas: R$ ${finance.gross.toFixed(2)}`, `Comissao: R$ ${finance.commission.toFixed(2)}`, `Valor liquido: R$ ${finance.net.toFixed(2)}`, `Pedidos concluidos: ${finance.orders}`, `Proximo repasse: ${new Date(finance.nextPayout).toLocaleDateString('pt-BR')}`],
      },
      {
        ...
        rows: [
          ...
          ...orders
            ...
            .map((o) => `${o.id} | ${o.customerName} | R$ ${Number(o.total).toFixed(2)} | ${new Date(o.createdAt).toLocaleDateString('pt-BR')}`)
            .slice(0, 250),
          ...(!orders.some((o) => o.status === 'delivered') ? ['Nenhum pedido entregue no periodo.'] : []),
        ],
      },
    ])
  if (type === 'pedidos')
    return createSimplePdf('Relatorio de pedidos', `${store.name} | ${orders.length} registros`, [
      {
        title: 'Pedidos',
        rows: ['#Pedido | Cliente | Status | Valor | Data e hora', ...orders.map((o) => `${o.id} | ${o.customerName} | ${o.status} | R$ ${Number(o.total).toFixed(2)} | ${new Date(o.createdAt).toLocaleString('pt-BR')}`).slice(0, 300), ...(orders.length ? [] : ['Nenhum pedido registrado.'])],
      },
    ])
  if (type === 'promocoes')
    return createSimplePdf('Relatorio de promocoes', `${store.name} | ${promotions.length} campanhas`, [
      {
        title: 'Campanhas',
        rows: ['#Campanha | Beneficio | Status | Utilizacoes', ...promotions.map((p) => `${p.name} | ${p.type === 'fixed' ? 'R$ ' + Number(p.value).toFixed(2) : p.value + '%'} | ${p.active ? 'Ativa' : 'Pausada'} | ${p.uses || 0} usos`), ...(promotions.length ? [] : ['Nenhuma promocao cadastrada.'])],
      },
    ])
  return createSimplePdf('Visao geral da operacao', `${store.name} | ${date}`, [
    {
      title: 'Indicadores',
      rows: [`Loja: ${store.open ? 'Aberta' : 'Pausada'}`, `Pedidos totais: ${orders.length}`, `Pedidos pendentes: ${orders.filter((o) => o.status === 'pending').length}`, `Receita entregue: R$ ${finance.gross.toFixed(2)}`, `Produtos cadastrados: ${store.products.length}`, `Promocoes ativas: ${promotions.filter((p) => p.active).length}`],
    },
  ])
}
...
async function analyzeMenuImage(store, image) {
  if (!process.env.OPENAI_API_KEY) {
    const error = new Error('A leitura por IA ainda não foi ativada neste ambiente. Configure OPENAI_API_KEY no Railway.')
...
api['POST /api/partner-subscription-pix'] = async (params, query, body, ctx) => {
  ...
  platform.audit(ctx.user, 'subscription.pix.create', 'subscription', subscription.id, txid)
  db.saveNow()
  return {
    payload,
    qrCode,
...
Object.assign(api, {
  'GET /api/cep/:id': async (params) => {
    ...
    try {
      ...
      if (payload.erro) return { status: 404, body: { error: 'CEP não encontrado.' } }
      const address = {
        cep: payload.cep,
  ...
  'POST /api/auth/partner-register': (params, query, body, ctx) => {
    ...
    if (existing && platform.storeForUser(existing))
      return {
        status: 409,
        body: {
          error: 'Esta conta já possui um estabelecimento. Entre pelo Portal do Parceiro.',
          code: 'STORE_EXISTS',
        },
      }
    if (existing && !auth.verifyPassword(pw.value, existing.passwordHash))
      return {
        status: 401,
        body: {
          error: 'Para vincular sua conta existente, informe a mesma senha usada no FoodCourt.',
    ...
    const store = {
      ...
      deliveryModes: Array.isArray(body.deliveryModes) ? body.deliveryModes.filter((item) => ['delivery', 'pickup'].includes(item)) : ['delivery'],
  ...
  'POST /api/partner-product': (params, query, body, ctx) => {
    ...
    platform.audit(ctx.user, body.id ? 'product.update' : 'product.create', 'product', product.id)
    return { product }
  },
    ...
    try {
      ...
      if (!products.length)
        return {
          status: 422,
          body: {
            error: 'Não encontramos produtos legíveis. Tente uma foto mais nítida e bem iluminada.',
          },
        }
      return {
        analysis: {
          establishmentType: auth.sanitize(analysis.establishmentType).slice(0, 80),
          warnings: (analysis.warnings || []).map((item) => auth.sanitize(item).slice(0, 180)).slice(0, 10),
          products,
        },
      }
    } catch (error) {
  ...
  'POST /api/partner-menu-import': (params, query, body, ctx) => {
    ...
    const existing = new Set(store.products.map((item) => normalize(`${item.category}|${item.name}`))),
      imported = []
    ...
    platform.audit(ctx.user, 'menu.import', 'store', store.id, `${imported.length} produtos em rascunho`)
  ...
  'POST /api/partner-promotion': (params, query, body, ctx) => {
    const store = platform.storeForUser(ctx.user),
      ...
      minimumOrder = Math.max(0, Math.min(100000, Number(body.minimumOrder) || 0)),
      startsAt = body.startsAt && Number.isFinite(Date.parse(body.startsAt)) ? new Date(body.startsAt + 'T00:00:00').toISOString() : platform.now(),
      endsAt = body.endsAt && Number.isFinite(Date.parse(body.endsAt)) ? new Date(body.endsAt + 'T23:59:59').toISOString() : null
    if (!name || !value)
      return {
        status: 400,
        body: {
          error: type === 'percent' ? 'Informe o nome e um desconto entre 1% e 90%.' : 'Informe o nome e um desconto em reais válido.',
        },
      }
    ...
    if (code && db.state.promotions.some((p) => p.storeId === store.id && p.id !== body.id && p.code === code))
      return {
        status: 409,
        body: { error: 'Este código já está sendo usado em outra promoção.' },
      }
    let promotion = db.state.promotions.find((p) => p.id === body.id && p.storeId === store.id)
    const changes = {
      name,
      code,
      type,
    ...
    platform.audit(ctx.user, body.id ? 'promotion.update' : 'promotion.create', 'promotion', promotion.id)
    return { promotion }
  },
  ...
  'POST /api/partner-team-member': (params, query, body, ctx) => {
    const store = platform.storeForUser(ctx.user)
    let member = db.state.storeMembers.find((m) => m.id === body.id && m.storeId === store.id)
    if (body.action === 'remove') {
      if (!member) return { status: 404, body: { error: 'Pessoa não encontrada.' } }
      db.state.storeMembers = db.state.storeMembers.filter((m) => m.id !== member.id)
    ...
    if (!member) {
      if (db.state.storeMembers.some((m) => m.storeId === store.id && m.email.toLowerCase() === email))
        return {
          status: 409,
    ...
    platform.audit(ctx.user, body.id ? 'team.update' : 'team.invite', 'member', member.id)
    return { member }
  },
  ...
  'POST /api/partner-review-reply': (params, query, body, ctx) => {
    const store = platform.storeForUser(ctx.user),
      review = db.state.reviews.find((r) => r.id === body.reviewId && r.storeId === store.id),
      reply = auth.sanitize(body.reply).slice(0, 500)
    if (!review) return { status: 404, body: { error: 'Avaliação não encontrada.' } }
    if (reply.length < 2)
      return {
        status: 400,
  ...
  'GET /api/partner-report/:id': (params, query, body, ctx) => {
    const store = platform.storeForUser(ctx.user),
      type = ['geral', 'pedidos', 'promocoes', 'financeiro'].includes(params.id) ? params.id : 'geral',
  ...
  'POST /api/partner-support-ticket': (params, query, body, ctx) => {
    const store = platform.storeForUser(ctx.user)
    let ticket = db.state.supportTickets.find((t) => t.id === body.id && t.storeId === store.id)
    if (ticket) {
  ...
  'POST /api/orders': (params, query, body, ctx) => {
    ...
    const partnerStore = platform.storeForId(body.storeId) || db.state.stores.find((item) => item.slug === body.storeId)
  ...
  'GET /api/customer-reviews': (params, query, body, ctx) => ({
    reviews: db.state.reviews.filter((review) => review.customerId === ctx.user.id),
  }),
  'POST /api/customer-reviews': (params, query, body, ctx) => {
    const order = db.state.platformOrders.find((item) => item.id === body.orderId && item.customerId === ctx.user.id && item.status === 'delivered')
    if (!order)
      return {
        status: 400,
        body: { error: 'Apenas pedidos entregues podem ser avaliados.' },
      }
    if (db.state.reviews.some((review) => review.orderId === order.id)) return { status: 409, body: { error: 'Este pedido já foi avaliado.' } }
  ...
  'GET /api/loyalty': (params, query, body, ctx) => {
    ...
    const next = levels[levels.findIndex((item) => item.name === level.name) + 1] || null
    return {
      points,
      level: level.name,
      next,
      ...
      missions: [
        ...
        },
        {
          ...
          progress: db.state.platformOrders.filter((o) => o.customerId === ctx.user.id).length,
          target: 5,
          reward: 150,
        },
        {
          ...
          progress: db.state.reviews.some((r) => r.customerId === ctx.user.id) ? 1 : 0,
          target: 1,
          reward: 10,
        },
      ],
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname.startsWith('/api/')) {
    ...
    try {
      ...
      const result = await route.handler(route.params, url.searchParams, body, ctx)
      ...
      console.error('[api]', e)
      sendJson(res, 500, {
        error: 'Não conseguimos concluir sua solicitação agora. Tente novamente.',
      })
    }
    return
  }
...
function start(port = PORT) {
  server.listen(port, () => {
    ...
    if (seedDemoData) console.log(`  Conta demo: joao@foodcourt.com / foodcourt123`)
    console.log('')
  })
  return server
}
