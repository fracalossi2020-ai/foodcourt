...
const routes = [
  ...
  {
    pattern: /^\/para-estabelecimentos$/,
    page: "partner-marketing",
    public: true,
    mode: "marketing",
  },
  {
    pattern: /^\/cadastro-parceiro$/,
    page: "partner-marketing",
    public: true,
    mode: "register",
  },
...
async function navigate() {
  ...
  try {
    ...
    const mod = await import(
      `./pages/${route.page}.js?v=20260904-admin-audit-1`
    );
    currentPage = mod;
    window.scrollTo(0, 0);
    ...
    const isSessionError =
      error?.status === 401 ||
      /autentic|sessão|sessao/i.test(error?.message || "");
    if (isSessionError && !isAuthPath(target)) {
      ...
      view
        .querySelector("[data-route-retry]")
...
function getBoot() {
  if (!bootPromise) {
    bootPromise = api.bootstrap().then((b) => {
      hydrateBootstrap(b);
      syncHeader();
      return b;
    });
...
function updateNav(path, query = new URLSearchParams()) {
  ...
  if (logo)
    logo.href = document.body.classList.contains("app-mode")
      ? "#/inicio"
      : "#/";
}
...
function enhanceInternalView(view) {
  const targets = view.querySelectorAll(
    ".consumer-section-head, .section-head, .restaurant-row, .product-row, .offer-strip, .nearby-list, .grid-rest, .card:not(.rcard):not(.pcard)",
  );
  if (
...
function renderLocDrawer() {
  ...
  drawer
    ...
    .forEach((b) => b.addEventListener("click", () => hide("locDrawer")));
  drawer.querySelectorAll("[data-addr]").forEach((b) =>
  ...
  drawer.querySelector("[data-newaddr]")?.addEventListener("click", () => {
    hide("locDrawer");
    location.hash = "#/perfil?secao=enderecos";
  });
  drawer
    .querySelector(".location-manage")
    ?.addEventListener("click", () => hide("locDrawer"));
}
...
function wireHeader() {
  ...
  document.getElementById("locBtn").addEventListener("click", (event) => {
    ...
    const rect = event.currentTarget.getBoundingClientRect(),
      drawer = document.getElementById("locDrawer");
...
function wireGlobalHelp() {
  const root = document.querySelector(".app-global-help"),
    button = root?.querySelector(".fcv2-help-button"),
    panel = root?.querySelector(".fcv2-help-panel"),
    close = root?.querySelector(".fcv2-help-close");
