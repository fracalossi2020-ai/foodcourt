...
export function openProduct(restaurant, product) {
  ...
  drawer.innerHTML = `
    <div class="drawer-head">
      ...
      <button class="icon-btn" data-pd-close aria-label="Fechar">${icon("close")}</button>
    </div>
  ...
  drawer
    .querySelector("[data-pd-close]")
    .addEventListener("click", () => hide("productDrawer"));
  refreshAdd();
  show("productDrawer");

  addBtn.addEventListener("click", () => {
    const missing = product.options.findIndex(
      (g, gi) => g.required && !sel.get(gi)?.size,
    );
    if (missing >= 0) {
      toast(
        `Escolha uma opção em “${product.options[missing].name}”`,
        "error",
        "⚠️",
      );
      drawer
        .querySelector(`[data-group="${missing}"]`)
