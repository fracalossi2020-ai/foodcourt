...
async function get(path, { ttl = 0 } = {}) {
  ...
  if (!res.ok)
    throw new ApiError(
      body.error || `Falha ao carregar (${res.status})`,
      res.status,
      body,
    );
...
async function post(path, payload) {
  ...
  if (res.status === 401) {
    unauthorized();
    throw new ApiError(body.error || "Não autenticado.", 401, body);
  }
  if (!res.ok)
    throw new ApiError(
      body.error ||
...
export const api = {
  ...
  importPartnerMenu: (products) =>
    post("/api/partner-menu-import", { products }),
  ...
  updateCourierDelivery: (deliveryId, action) =>
    post("/api/courier-delivery", { deliveryId, action }),
  requestCourierWithdrawal: (amount, pixKey) =>
    post("/api/courier-withdrawal", { amount, pixKey }),
  ...
  sendOrderMessage: (orderId, text) =>
    post("/api/chat-message", { orderId, text }),
  cancelOrder: (orderId, reason) =>
    post("/api/order-cancel", { orderId, reason }),
  customerReviews: () => get("/api/customer-reviews"),
  createReview: (review) => post("/api/customer-reviews", review),
  loyalty: () => get("/api/loyalty"),
  redeemLoyalty: (rewardId, redemptionKey) =>
    post("/api/loyalty-redeem", { rewardId, redemptionKey }),
