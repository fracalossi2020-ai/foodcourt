...
function teamContent(data) {
  ...
  return `${head('PESSOAS', 'Equipe da loja', 'Organize os colaboradores e mantenha seus cadastros atualizados.', '<button class="btn btn-primary" data-new-member>+ Adicionar pessoa</button>')}
    ...
    <div class="team-directory-list">${members.map(member => `<article class="team-person" data-team-person="${esc(member.id)}" data-role="${esc(member.role)}" data-status="${member.active === false ? 'inactive' : 'active'}"><span class="team-avatar" aria-hidden="true">${esc(member.name.slice(0, 2).toUpperCase())}</span><div class="team-person-info"><h3>${esc(member.name)}</h3><p>${esc(member.email)}</p><span>${esc(roleLabel(member.role))}</span> <b class="team-status ${member.active === false ? 'inactive' : ''}">${member.active === false ? 'Inativo' : 'Ativo'}</b></div><div class="team-person-actions">${member.active !== false && member.userId ? `<button class="btn btn-outline" data-email-member="${esc(member.id)}">Enviar acesso por e-mail</button>` : ""}<button class="btn btn-outline" data-edit-member="${esc(member.id)}" aria-label="Editar ${esc(member.name)}">Editar</button><button class="btn btn-outline" data-toggle-member="${esc(member.id)}">${member.active === false ? 'Reativar' : 'Desativar'}</button></div></article>`).join('')}</div><div class="team-empty" data-team-empty ${members.length ? 'hidden' : ''}><h3>${members.length ? 'Nenhum resultado' : 'Sua equipe começa aqui'}</h3><p>${members.length ? 'Tente outro nome ou ajuste os filtros.' : 'Adicione as pessoas que trabalham com você.'}</p></div></section><p class="team-help">A pessoa deve criar uma conta FoodCourt antes de ser adicionada. Cadastros antigos precisam ser editados e salvos para vincular a conta. Após vincular a conta, use Enviar acesso por e-mail para enviar as instruções. O envio depende da configuração de e-mail do servidor.</p>`;
}
...
function bind(view, section, data) {
  ...
  view.querySelectorAll('[data-email-member]').forEach(button => button.addEventListener('click', async () => { button.disabled = true; try { await api.emailPartnerTeamMember(button.dataset.emailMember); toast('Instruções enviadas ao e-mail do colaborador.', 'success'); } catch (error) { toast(error.message, 'error'); } finally { button.disabled = false; } }));
  ...
  view.querySelectorAll("[data-product-active]").forEach((input) =>
    input.addEventListener("change", async () => {
      ...
      if (!product) return;
      if (input.checked && (!(Number(product.price) > 0) || !(Number(product.stock) > 0))) {
        input.checked = false;
        toast("Defina um preço maior que zero e informe o estoque antes de disponibilizar.", "error");
        view.querySelector(`[data-edit-product="${product.id}"]`)?.click();
        return;
      }
      input.disabled = true;
      try {
