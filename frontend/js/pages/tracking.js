...
import { updateDeliveryMap } from '../core/delivery-map.js'
...
export async function render(view,boot,params){
  ...
  try{messages=(await api.orderChat(params.id)).messages}catch{}
  ...
  const stages=order.fulfillment==='pickup' ? [['pending','Pedido recebido','Aguardando a loja.'],['accepted','Pedido aceito','A loja confirmou o pedido.'],['preparing','Em preparação','Seu pedido está sendo preparado.'],['ready','Pronto para retirada','Busque seu pedido no endereço da loja.'],['delivered','Retirado','Retirada confirmada pela loja.']] : deliveryStages
  let review
  try { review = (await api.customerReviews()).reviews.find(item => item.orderId === order.id) } catch {}
  ...
  function draw(){
    ...
    if (order.paymentIntentId) {
      ...
      items.style.cssText = 'padding:18px;margin-top:16px';
      items.innerHTML = `<h2>Itens do pedido</h2>${(order.items || []).map(item => `<article style="margin-top:12px"><b>${item.quantity} × ${esc(item.name)}</b>${item.options?.length ? '<p>' + item.options.map(esc).join(', ') + '</p>' : ''}${item.note ? '<p>Observação: ' + esc(item.note) + '</p>' : ''}</article>`).join('')}`;
      root.querySelector('.tracking-status').after(items);
      const banner = document.createElement('div')
      banner.className = 'card'
      banner.style.padding = '18px'
      banner.style.marginTop = '16px'
      const label = ({ paid: 'Pagamento aprovado', pending: 'Aguardando pagamento', failed: 'Pagamento n\u00e3o aprovado', refund_pending: 'Estorno em processamento', refunded: 'Pagamento estornado', cancelled: 'Pagamento cancelado', expired: 'Pagamento expirado' })[order.paymentStatus] || 'Ver pagamento'
      banner.innerHTML = `<b>${label}</b><a class="btn btn-outline btn-sm" href="#/checkout?payment=${encodeURIComponent(order.paymentIntentId)}">Ver pagamento</a>`
