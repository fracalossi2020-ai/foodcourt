...
export async function render(view, _boot) {
  ...
  let tab = "restaurants";
  const allProducts = [];
  products.filter(Boolean).forEach((r) =>
    r.menu.forEach((s) =>
      s.items.forEach((p) => {
  ...
  function draw() {
    const favRests = all.restaurants.filter((r) =>
      store.isFavoriteRestaurant(r.id),
    );
    ...
    <div class="page account-destination-page">
      ...
      <header class="destination-heading"><span class="destination-icon">${icon("heart")}</span><div><span class="account-kicker">MINHA CONTA</span><h1>Favoritos</h1><p>Todos os sabores que você salvou em um só lugar.</p></div></header>
      ...
      <div class="tabs modern-tabs">
        <button class="chip ${tab === "restaurants" ? "active" : ""}" data-tab="restaurants">${icon("store")} Restaurantes (${favRests.length})</button>
        <button class="chip ${tab === "products" ? "active" : ""}" data-tab="products">${icon("menu")} Produtos (${favProds.length})</button>
      </div>
      ${
        tab === "restaurants"
          ? favRests.length
            ...
            : emptyState({
                emoji: "🤍",
                title: "Você ainda não favoritou nenhum restaurante",
                sub: "Toque no coração dos cards para salvar seus preferidos e encontrá-los aqui.",
                action: "#/",
          ...
          : favProds.length
            ...
            : emptyState({
                ...
                sub: "Favorite produtos para reencontrá-los rapidinho.",
                action: "#/buscar",
