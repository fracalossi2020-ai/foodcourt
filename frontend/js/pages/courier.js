  ...
  try {
    ...
    const withdrawals =
      data.withdrawals
        .map(
          (item) =>
            `<div class="admin-row"><span>💸</span><div><b>${money(item.netAmount ?? item.amount)} líquido</b><small>Taxa FoodCourt ${money(item.platformFee || 0)} · ${new Date(item.requestedAt).toLocaleString("pt-BR")}</small></div><em>${payoutLabel[item.status] || esc(item.status)}</em></div>`,
        )
        .join("") || '<p class="partner-empty">Nenhum saque solicitado.</p>';
    view.innerHTML = `<div class="page admin-page courier-page"><header class="admin-head"><div><span>FOODCOURT ENTREGAS</span><h1>Olá, ${esc(data.profile.name.split(" ")[0])}</h1><p>Gerencie disponibilidade, convites, entregas e ganhos em uma área exclusiva.</p></div><button class="btn ${data.profile.available ? "btn-primary" : "btn-outline"}" data-availability>${data.profile.available ? "● Disponível" : "Ficar disponível"}</button></header><section class="partner-metrics"><article class="partner-metric"><i>💰</i><span>Ganhos totais</span><b>${money(data.earnings)}</b><small>entregas concluídas</small></article><article class="partner-metric"><i>🏦</i><span>Saldo disponível</span><b>${money(data.availableBalance)}</b><small>livre para saque</small></article><article class="partner-metric"><i>⭐</i><span>Avaliação</span><b>${data.profile.rating.toFixed(1)}</b><small>qualidade do serviço</small></article><article class="partner-metric"><i>🛵</i><span>Veículo</span><b>${esc(data.profile.vehicle)}</b><small>modalidade cadastrada</small></article></section>${current ? `<section class="partner-panel"><header><h2>Entrega atual</h2><em>${esc(current.status)}</em></header><div class="courier-route"><article><small>RETIRADA</small><b>${esc(place(current.pickupAddress) || "Estabelecimento")}</b></article><span>→</span><article><small>ENTREGA</small><b>${esc(place(current.dropoffAddress) || "Endereço do cliente")}</b></article></div><div class="pair">${actionFor(current) ? `<button class="btn btn-primary" data-delivery-action="${actionFor(current)[0]}" data-delivery-id="${current.id}">${actionFor(current)[1]}</button>` : ""}<a class="btn btn-outline" href="#/conversa/${current.orderId}">Conversar</a></div></section>` : ""}<section class="partner-panel"><header><h2>Convites de entrega</h2><span>${data.available.length}</span></header>${data.available.length ? data.available.map((item) => `<div class="admin-row"><span>📦</span><div><b>${esc(place(item.dropoffAddress) || "Nova entrega")}</b><small>Retirada em ${esc(place(item.pickupAddress) || "estabelecimento")} · ${item.commissionPercent}% do pedido</small></div><strong>${money(item.courierPayout)}</strong><button class="btn btn-primary btn-sm" data-delivery-action="accept" data-delivery-id="${item.id}">Aceitar</button><button class="btn btn-ghost btn-sm" data-delivery-action="decline" data-delivery-id="${item.id}">Recusar</button></div>`).join("") : '<p class="partner-empty">Quando uma loja convidar você, a proposta aparecerá aqui.</p>'}</section><section class="partner-panel"><header><h2>Saques via Pix</h2><button class="btn btn-primary btn-sm" data-withdraw ${data.availableBalance < 10 ? "disabled" : ""}>Solicitar saque</button></header>${withdrawals}</section><section class="partner-panel"><header><h2>Histórico de entregas</h2></header>${data.history.map((item) => `<div class="admin-row"><span>✓</span><div><b>${esc(item.orderId)}</b><small>${new Date(item.updatedAt).toLocaleString("pt-BR")}</small></div><strong>${money(item.courierPayout)}</strong></div>`).join("") || '<p class="partner-empty">As entregas concluídas aparecerão aqui.</p>'}</section></div>`;
    stopLocation = mountLocation(view, current);
    mountNavigation(view, current);
    view
    ...
    view
      ...
      ?.addEventListener("click", async (event) => {
        ...
        if (
          !confirm(
            `A taxa FoodCourt é de 5%. Você receberá ${money(Number(amount.replace(",", ".")) * 0.95)}. Continuar?`,
          )
        )
          return;
