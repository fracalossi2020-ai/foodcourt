...
export function openCart() { render(); show('cartDrawer') }
export function closeCart() { hide('cartDrawer') }
...
export function hide(id) {
  ...
  if (![...document.querySelectorAll('.drawer.open')].filter(d => d.id !== id).length) {
...
export function setFeeContext(fee, freeMin) { feeCtx = { fee, freeMin } }
...
function render() {
  ...
  drawer.innerHTML = `
    <div class="drawer-head">
      ...
      <button class="icon-btn" data-close-cart aria-label="Fechar carrinho">✕</button>
    </div>
  ...
  drawer.querySelectorAll('[data-goto]').forEach(b => b.addEventListener('click', () => { closeCart(); location.hash = b.dataset.goto }))
  drawer.querySelectorAll('[data-qty-minus]').forEach(b => b.addEventListener('click', () => { store.cartUpdateQty(b.dataset.qtyMinus, -1); renderCartUI() }))
  drawer.querySelectorAll('[data-qty-plus]').forEach(b => b.addEventListener('click', () => { store.cartUpdateQty(b.dataset.qtyPlus, 1); renderCartUI() }))
...
function applyCoupon() {
  ...
  if (!def) { toast('Cupom inválido ou expirado', 'error', '⚠️'); return }
  const t = store.cartTotals(feeCtx.fee, feeCtx.freeMin)
  if (t.subtotal < (def.min || 0)) { toast(`Pedido mínimo de ${money(def.min)} para este cupom`, 'error', '⚠️'); return }
...
export function renderCartUI() {
  ...
  if (n > 0) { dot.hidden = false; dot.textContent = n; dot.style.animation = 'none'; requestAnimationFrame(() => dot.style.animation = '') }
  ...
  if (n > 0) {
    const t = store.cartTotals(feeCtx.fee, feeCtx.freeMin)
    if (!bar) {
      bar = el(`<button class="cartbar" id="cartbar" aria-label="Abrir carrinho"></button>`)
    ...
    if (shouldBump) { bar.classList.remove('bump'); void bar.offsetWidth; bar.classList.add('bump') }
