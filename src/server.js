...
Object.assign(api, {
  ...
  'POST /api/partner-promotion': (params,query,body,ctx) => {const store=platform.storeForUser(ctx.user);const name=auth.sanitize(body.name).slice(0,80),value=Math.max(1,Math.min(90,Number(body.value)||0));if(!name||!value)return {status:400,body:{error:'Informe o nome e um benefício entre 1% e 90%.'}};let promotion=db.state.promotions.find(p=>p.id===body.id&&p.storeId===store.id);if(promotion)Object.assign(promotion,{name,value,active:body.active!==false});else{promotion={id:db.uid('promo'),storeId:store.id,name,type:'percent',value,active:true,uses:0,startsAt:platform.now(),endsAt:null};db.state.promotions.unshift(promotion)}platform.audit(ctx.user,body.id?'promotion.update':'promotion.create','promotion',promotion.id);return {promotion}},
